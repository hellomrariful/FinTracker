...
export const ExpenseFiltersSchema = z.object({
  ...
  sortBy: z
    .enum(["date", "amount", "name", "vendor", "createdAt"])
    .default("date"),
...
export class ExpenseRepository {
  ...
  async find(userId: string, filters: ExpenseFilters) {
    ...
    if (filters.isReimbursable !== undefined)
      query.isReimbursable = filters.isReimbursable;
    if (filters.reimbursementStatus)
      query.reimbursementStatus = filters.reimbursementStatus;
    if (filters.taxDeductible !== undefined)
      query.taxDeductible = filters.taxDeductible;
    if (filters.approvedBy)
      query.approvedBy = new mongoose.Types.ObjectId(filters.approvedBy);
      ...
      { $match: query },
      {
        $group: {
          ...
          reimbursementPendingAmount: {
            $sum: {
              $cond: [
                { $eq: ["$status", "reimbursement_pending"] },
                "$amount",
                0,
              ],
            },
  ...
  async approveReimbursement(
    ...
    if (notes) {
      updateData.$push = {
        notes: { text: notes, date: new Date(), userId: approverId },
      };
    }
  ...
  async getEmployeeExpenses(
    ...
    const stats = {
      ...
      approved: expenses.filter((e) => e.reimbursementStatus === "approved")
        .length,
    };
