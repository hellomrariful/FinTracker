...
export const IncomeFiltersSchema = z.object({
  ...
  sortBy: z.enum(['date', 'amount', 'name', 'source', 'createdAt']).default('date'),
...
export class IncomeRepository {
  ...
  async find(userId: string, filters: IncomeFilters) {
    ...
    if (filters.recurringFrequency) query.recurringFrequency = filters.recurringFrequency;
  ...
  async update(userId: string, incomeId: string, data: z.infer<typeof UpdateIncomeSchema>) {
    await this.ensureConnection();
